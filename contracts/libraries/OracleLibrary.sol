...
library OracleLibrary {
    ...
    /// @notice Fetches time-weighted average tick using BitrielSwap oracle
    /// @param pool Address of Bitriel pool that we want to observe
    ...
    function consult(address pool, uint32 period) internal view returns (int24 timeWeightedAverageTick) {
        ...
        (int56[] memory tickCumulatives, ) = IBitrielPool(pool).observe(secondAgos);
        int56 tickCumulativesDelta = tickCumulatives[1] - tickCumulatives[0];

        timeWeightedAverageTick = int24(int256(tickCumulativesDelta) / int256(uint256(period)));
    ...
    function getQuoteAtTick(
        ...
        if (sqrtRatioX96 <= type(uint128).max) {
            ...
            quoteAmount = baseToken < quoteToken
                ? ratioX192.mulDiv(baseAmount, 1 << 192)
                : uint256(1 << 192).mulDiv(baseAmount, ratioX192);
        } else {
            uint256 ratioX128 = uint256(sqrtRatioX96).mulDiv(sqrtRatioX96, 1 << 64);
            quoteAmount = baseToken < quoteToken
                ? ratioX128.mulDiv(baseAmount, 1 << 128)
                : uint256(1 << 128).mulDiv(baseAmount, ratioX128);
        }
    }
}
