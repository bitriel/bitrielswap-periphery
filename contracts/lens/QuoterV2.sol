...
contract QuoterV2 is IQuoterV2, IBitrielSwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IBitrielPool;
    ...
    function swapCallback(
        ...
        IBitrielPool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IBitrielPool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
        ...
        )
    {
        ...
        try
            pool.swap(
                address(this), // address(0) might cause issues with some tokens
                zeroForOne,
                int256(type(uint256).max - params.amount + 1),
                params.sqrtPriceLimitX96 == 0
                    ? (zeroForOne ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1)
                    : params.sqrtPriceLimitX96,
